...
public class MyFirstActivity extends Activity {
		...
		bookstoreGridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				case kBookStateEnum_Unziping:// 解压书籍zip资源包中....
					Toast.makeText(MyFirstActivity.this, "正在解压中。。。。。。", Toast.LENGTH_SHORT).show();
					break;
				case kBookStateEnum_Installed:// 已安装(已经解压开的书籍, 可以正常阅读了)
					// GlobalDataCacheForMemorySingleton.getInstance.getLocalBookList();

					// LocalCacheDataPathConstant.localBookCachePath() ....LocalBookDir
					// book.getBookInfo().getContent_id();
	...
	private void readDocument(final File file) {
		Intent intent = new Intent();
		intent.setClass(MyFirstActivity.this, SecondActivity.class);
		intent.putExtra(SecondActivity.EXTRA_ZIP_FILE, file.getPath());
		startActivity(intent);
	}
}
